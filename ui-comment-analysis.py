...
df.to_csv("output/ui-comments-summary.csv", index=False, header=True)
...
df.groupby(['cluster_label', 'cluster_top_terms']).count().reset_index().sort_values(by=['cluster_label']).to_csv('output/ui-comments-clusters.csv',index=False, header=True, columns=header)
